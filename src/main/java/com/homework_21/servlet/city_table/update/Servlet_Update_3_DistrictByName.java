...
@WebServlet("/update/3")
public class Servlet_Update_3_DistrictByName extends HttpServlet {
    JDBC_CRUD_Manager manager = JDBC_CRUD_Manager.getJdbc_crud_manager();

    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        PrintWriter pw = response.getWriter();
        final String name = request.getParameter("name");
        final String newDistrict = request.getParameter("newDistrict");
        if (manager.isNameContainsInDatabase(name)){
            manager.updateDistrictByCityName(name, newDistrict);
            response.sendRedirect("/HomeWork_Lesson_21_war_exploded/update/3");
        } else {
            pw.println("<html>");
            ...
            pw.println("<form align=\"center\" action=\"/HomeWork_Lesson_21_war_exploded/update/3\">");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        pw.println("<p><b> new district: </b><br>  <input type=\"text\" name = \"newDistrict\" size=" + 20 + "required pattern=\"[A-Za-z]*\">");
